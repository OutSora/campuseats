...
```mermaid
flowchart TB
    student["Студент"] -->|"смотрит меню, создает заказ"| system["CampusEats"]
    kitchen["Сотрудник кухни"] -->|"получает и готовит заказы"| system
    operator["Оператор"] -->|"смотрит уведомления и аналитику"| system
```

Граница системы — весь CampusEats; внешних платежных/SMS-провайдеров в MVP нет.
