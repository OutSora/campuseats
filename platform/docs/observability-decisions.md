...
CampusEats сейчас не вызывает LLM, поэтому искусственно добавлять AI-monitoring нецелесообразно. Если появится сервис рекомендаций на LLM, рассматриваются Langfuse или OpenLLMetry для token usage, latency, cost, prompt/version и quality scores; телеметрия передаётся через OpenTelemetry Collector.
